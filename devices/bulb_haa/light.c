...
uint8_t setup_mode_toggle_counter = 0;
...
void setup_mode_call(const uint8_t gpio) {
    ...
    if (xTaskGetTickCountFromISR() < ALLOWED_SETUP_MODE_TIME / portTICK_PERIOD_MS) {
        //LED_BLINK(4);
        light_code();
        printf("HAA ! Enter Setup mode within %i msecs since boot. REBOOT NOW \n", ALLOWED_SETUP_MODE_TIME);
...
void light_code() {
    printf("Light Code\n");
    //xTaskCreate(light_code_task, "Light code", 256, NULL, 2, NULL);
    xTaskCreate(light_code_task, "light code", configMINIMAL_STACK_SIZE, NULL, 1, NULL);
}
...
void user_init(void) {
    ...
    if (status == SYSPARAM_OK && haa_setup == true) {
        uart_set_baud(0, 115200);
        printf("\n\nHAA > Home Accessory Architect\nHAA > Developed by @TrioSystem - Roy Suter\nHAA > Version: %s\n\n", FW_VERSION);
