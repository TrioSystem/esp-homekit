...
static void form_params_get(form_param_t *params, const char *name) {
    sysparam_status_t status;
    bool bool_value;
    int8_t int8_value;
    
    status = sysparam_get_bool(name, &bool_value);
    if (status == SYSPARAM_OK) {
        if (params) {
            memset(params, 0, sizeof(*params));
            strcpy(params->name, (char *)name);
            strcpy(params->value, (char *)bool_value ? "off" : "on");
            //params = params->next;
        }
        
        
    }else {
        status = sysparam_get_int8(name, &int8_value);
        if (status == SYSPARAM_OK) {
            //if (!strcmp(params->value, int8_value))
            //return params;
            //params = params->next;
...
static void wifi_config_server_on_settings(client_t *client) {
    ...
    client_send_chunk(client, html_settings_parameters);

    
    parameter_info_t *parameters = NULL;
    ...
    if (status == SYSPARAM_OK) {
        ...
        parameter_info_t *par = parameters;
        if (!par) {
            parameter_info_t *par = malloc(sizeof(parameter_info_t));
            memset(par, 0, sizeof(*par));
            strncpy(par->name, (char *)"show_setup", sizeof(par->name));
            
            par->value = bool_value;
            //par->state = int8_value;
            par->next = parameters;
            parameters = par;
        }
     } else {
         status = sysparam_get_int8("show_setup", &int8_value);
         if (status == SYSPARAM_OK) {
             parameter_info_t *par = parameters;
             if (!par) {
                 parameter_info_t *par = malloc(sizeof(parameter_info_t));
                 memset(par, 0, sizeof(*par));
                 strncpy(par->name, (char *)"init_state_sw1", sizeof(par->name));
                 //strncpy(par->state, (char *) int8_value, sizeof(par->name));
    ...
    //form_param_t *parameters1 = NULL;
   
    //form_params_get(parameters1, "show_setup");
    //printf(" >>> factory reset parameter name is -> %s\n", parameters1->name);
    //printf(" >>> factory reset parameter is -> %s\n", parameters1->value);

    
    
    char buffer1[64];
    memset(buffer1, 0, sizeof buffer1); // empty buffer
    
    
    parameter_info_t *param = parameters;
    //form_param_t *param = parameters;
    while (param) {
        snprintf(
        ...
        client_send_chunk(client, buffer1);
        param = param->next;
    }
...
static void wifi_config_server_on_settings_update(client_t *client) {
    ...
    form_param_t *init_1_param = form_params_find(form, "init1");
    form_param_t *init_sw1_param = form_params_find(form, "init_sw1");
    form_param_t *show_setup_param = form_params_find(form, "show_setup");
    ...
    sysparam_set_bool("factory_reset", reset_param->value[0]-0x30);
    sysparam_set_bool("show_setup", show_setup_param->value[0]-0x30);
    if (init_1_param && init_1_param->value) sysparam_set_int8("init_state_1", init_1_param->value[0]-0x30);
    sysparam_set_int8("init_state_sw1", init_sw1_param->value[0]-0x30);
    form_params_free(form);
